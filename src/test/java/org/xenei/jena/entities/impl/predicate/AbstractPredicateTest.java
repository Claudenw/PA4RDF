...
public abstract class AbstractPredicateTest {
    ...
    protected void assertSame(EffectivePredicateBuilder builder, EffectivePredicate actual, Method method) {
         ...
         */
        Assert.assertEquals( method.toGenericString() + "\n Wrong ACTION type", expected.actionType(),
                actual.actionType() );
        Assert.assertEquals( method.toGenericString() + "\n Wrong COLLECTION type", expected.collectionType(),
                actual.collectionType() );
        Assert.assertEquals( method.toGenericString() + "\n Wrong emptyIsNull", expected.emptyIsNull(),
                actual.emptyIsNull() );
        Assert.assertEquals( method.toGenericString() + "\n Wrong impl", expected.impl(), actual.impl() );
        Assert.assertEquals( method.toGenericString() + "\n Wrong INTERNAL type", expected.internalType(),
                actual.internalType() );
        Assert.assertEquals( method.toGenericString() + "\n Wrong LITERAL type", expected.literalType(),
                actual.literalType() );
        Assert.assertEquals( method.toGenericString() + "\n Wrong name", expected.name(), actual.name() );
        Assert.assertEquals( method.toGenericString() + "\n Wrong namespace", expected.namespace(),
                actual.namespace() );
 ...
 * order Predicate : Getter Predicate : Other
    ...
    public static class EffectivePredicateBuilder {
        ...
        public EffectivePredicate build() {
             ...
             */
            return new EffectivePredicate( actionType, collectionType, emptyIsNull, impl, internalType, literalType,
                    name, namespace, postExec, type, upcase );
