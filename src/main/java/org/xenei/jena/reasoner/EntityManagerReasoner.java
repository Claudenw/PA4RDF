...
/**
 * A reasoner that attempts to perform the same duplicate checking that the Entity manager performs
 * when adding values for properties.
 *
 */
public class EntityManagerReasoner implements Reasoner, EntityManager.Listener
{
	...
	private enum MethodType { SINGLE, MULTI, CONFLICT };
	...
	private void processSubjectInfo( SubjectInfo si )
	{
		for (final PredicateInfo pi : si.getPredicates().stream()
				.filter( p -> p.getActionType().equals(ActionType.SETTER))
			...
			if (pi.getMethodName().startsWith( "set"))
			{
				if (typ != null) {
					if ( ! typ.equals( MethodType.SINGLE))
					{
						...
						LOG.info( String.format("Conflict detected for %s while processing %s", pi.getProperty(), pi ));
					}
				}
				else {	
	...
	public InfGraph bind(Graph data) throws ReasonerException
	{
		return new BaseInfGraph(data,this){
			...
			public ExtendedIterator<Triple> findWithContinuation(
					TriplePattern pattern, Finder continuation)
			{
