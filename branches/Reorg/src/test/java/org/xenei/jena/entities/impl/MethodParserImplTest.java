...
import org.xenei.jena.entities.testing.impl.ImplementedA;
...
public class MethodParserImplTest
{
	...
	{
		subjectInfo = new SubjectInfoImpl(ImplementedA.class);
		final Map<String, Integer> addCount = countAdders(ImplementedA.class.getMethods());
	...
	{		
		Method m = ImplementedA.class.getMethod( "setX", String.class );
		...
		pi = subjectInfo.getPredicateInfo( ImplementedA.class.getMethod( "getX" ) );
		...
		pi = subjectInfo.getPredicateInfo( ImplementedA.class.getMethod( "hasX") );
		...
		pi = subjectInfo.getPredicateInfo( ImplementedA.class.getMethod( "removeX"));
	...
	{		
		Method m = ImplementedA.class.getMethod( "getX");
		PredicateInfo pi = parser.parse( m, new EffectivePredicate( m ));
		assertNotNull( "getX not parsed",  pi );
		assertEquals( ActionType.GETTER, pi.getActionType());
		assertEquals( "getX", pi.getMethodName());
	...
	{		
		Method m = ImplementedA.class.getMethod( "hasX");
		PredicateInfo pi = parser.parse( m, new EffectivePredicate( m ));
		assertNotNull( "hasX not parsed",  pi );
		assertEquals( ActionType.EXISTENTIAL, pi.getActionType());
		assertEquals( "hasX", pi.getMethodName());
	...
	{		
		Method m = ImplementedA.class.getMethod( "removeX");
		PredicateInfo pi = parser.parse( m, new EffectivePredicate( m ));
		assertNotNull( "removeX not parsed",  pi );
		assertEquals( ActionType.REMOVER, pi.getActionType());
		assertEquals( "removeX", pi.getMethodName());
