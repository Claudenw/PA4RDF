 ...
 * the values.
 *
 * @see org.xenei.pa4rdf.bean.annotations.Predicate
 */
public class EffectivePredicate
{
	 ...
	 * 
	 * @param ep
	 *            the Effective predicate to copy.
	 */
	 ...
	 * 
	 * @param actionType
	 *            The action type (may not be null)
	 * @param collectionType
	 *            The collection class type (may be null)
	 * @param emptyIsNull
	 *            True if empty values should be considered as null resources.
	 * @param impl
	 *            True if the predicate fronts a concrete implementation.
	 * @param internalType
	 *            The class type of the object in the graph.
	 * @param literalType
	 *            The internal type of Class type is Literal.
	 * @param name
	 *            The name for the predicate.
	 * @param namespace
	 *            the namespace for the predicate
	 * @param postExec
	 *            a list of methods to execute at end of processing (may be
	 *            null)
	 * @param type
	 *            The class that is used in the user/api interface.
	 * @param upcase
	 *            true if the first character of the name should be upper case.
	 */
	public EffectivePredicate(ActionType actionType, Class<?> collectionType,
			boolean emptyIsNull, boolean impl, Class<?> internalType,
			RDFDatatype literalType, String name, String namespace,
			List<Method> postExec, Class<?> type, boolean upcase)
	{
	 ...
	 * 
	 * @param method
	 *            The method to build the effective predicate from.
	 */
	public EffectivePredicate(final Method method)
	{
		...
		if (method != null)
		{
			...
			switch (actionType)
			{
				case EXISTENTIAL:
					...
					if (!b)
					{
						try
						{
							b = method.getReturnType().equals(
									Boolean.class.getField("TYPE").get(null));
					...
					if (!b)
					{
						throw new RuntimeException(String.format(
								"%s does not return boolean or Boolean",
								method));
					}
					// fall through
				case REMOVER:
			...
			final Subject s = method.getDeclaringClass()
			...
			if (predicate != null)
			{
				if (StringUtils.isNotBlank(predicate.postExec()))
				{
					...
					try
					{
						final Class<?> argType = isCollection() ? collectionType
								: type;
						final Method peMethod = method.getDeclaringClass()
								.getMethod(mName, argType);
						if (actionType == ActionType.GETTER
								&& !argType.equals(peMethod.getReturnType()))
						{
							throw new RuntimeException(String.format(
									"%s does not return its parameter type",
									peMethod));
						}
					...
					} catch (final NoSuchMethodException e)
					{
						throw new RuntimeException(
								"Error parsing predicate annotation", e);
					...
					{
						throw new RuntimeException(
								"Error parsing predicate annotation action type",
								e);
					}
				}
			}
	 ...
	 * 
	 * Checks that type and internaleType are set and that literalType is set as
	 * required. Throws RuntimeException if there are any issues.
	 */
	public void checkValid()
	{
		 ...
		 */
		if ((this.type == null) || ((!actionType.allowsNull())
				&& this.type == Predicate.UNSET.class))
		{
			throw new RuntimeException(
					String.format("%s %s does not define the parameter type",
							actionType, name));
		}
		if (this.internalType == null)
		{
			throw new RuntimeException(
					String.format("%s %s does not define the internal type",
							actionType, name));
		}
		if (this.internalType.equals(Literal.class) && literalType == null)
		{
			throw new RuntimeException(String.format(
					"%s %s defines the internal type as Literal but no literal type",
					actionType, name));
		}
	}
	...
	/**
	 * Return a value if it is not null. if opt is Predicate.UNSET.class it is
	 * considered as null.
	 * 
	 * @param opt
	 *            the item to check.
	 * @param dflt
	 *            the default if opt is null.
	 * @return opt or null.
	 */
	...
	public EffectivePredicate merge(final EffectivePredicate predicate)
	{
		if (predicate != null)
		{
			...
			name(StringUtils.isBlank(predicate.name()) ? name
					: predicate.name());
			...
			if (Predicate.UNSET.class.equals(this.type))
			{
				...
				} else
				{
					...
					{
						if (predicate.internalType != null
								&& !Predicate.UNSET.class
										.equals(predicate.internalType))
						{
			...
			} else
			{
				if (!actionType.allowsNull())
				{
					...
					internalType = notNull(internalType,
							predicate.internalType);
	...
	/**
	 * Merge from another effective predicate that is considered an ancestor to
	 * this one.
	 * 
	 * @param predicate
	 *            the other predicate to merge.
	...
	/**
	 * Determine if this is a collection. it is a collection of the collection
	 * type is not null.
	 * 
	...
	public EffectivePredicate merge(final Predicate predicate)
	{
		if (predicate != null)
		{
			...
			{
				final TypeMapper typeMapper = TypeMapper.getInstance();
				literalType = typeMapper
						.getSafeTypeByName(predicate.literalType());
			}
	...
	/**
	 * 
	 * @return Get the name of the predicate this method represents in the
	 *         graph.
	 */
	public String name()
	{
	...
	/**
	 * Set the name for the predicate this method represents. The value of the
	 * name may be modified by the upcase flag.
	 * 
	 * @param name
	 *            the name to set the predicate to.
	 */
	...
	public String toString()
	{
		return String.format(
				...
				namespace, name, impl, type, internalType, collectionType,
				upcase, emptyIsNull, literalType,
	...
	public boolean equals(Object o)
	{
		...
		{
			final EffectivePredicate other = (EffectivePredicate) o;
			if (actionType == null)
			{
				return super.equals(o);
			}
			return new EqualsBuilder().append(actionType, other.actionType)
					.append(upcase, other.upcase).append(name, other.name)
					...
					.append(impl, other.impl).append(postExec, other.postExec)
					.build();
		}
		return false;
	}
	...
	public String formattedString()
	{
		return String.format(
				...
				actionType, collectionType, emptyIsNull, impl, internalType,
				literalType, name, namespace, type, upcase, postExec);
	}
